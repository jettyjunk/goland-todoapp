...
func NewTaskUnitialized(title string, description *string, authoreUserID int) Task {
	return NewTask(
		UnitializedID,
		UnitializedVersion,
		title,
...
func (t *Task) Validate() error {
	...
	if t.Completed {
		...
		if t.CompletedAt.Before(t.CreatedAt) {
			return fmt.Errorf("`CompletedAt` cant`t be before `CreatedAt`: %w",
				core_errors.ErrInvalidArgument,
			)
		}
	} else {
...
func NewTaskPatch(title Nullable[string], description Nullable[string], completed Nullable[bool]) TaskPath {
	return TaskPath{
		Title:       title,
...
func (t *Task) ApplyPatch(patch TaskPath) error {
